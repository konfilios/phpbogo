 ...
 */
class Query extends Webservice\Dto
{
	/**
		...
		if (!empty($this->sort)) {
			...
			foreach ($this->sort as $sort) {
				...
				if (!empty($sortFields[$fieldAlias])) {
					...
					throw new \CHttpException(400, 'Unknown sort field "'.$fieldAlias.'". Valid sort fields are: '.implode(', ', array_keys($sortFields)), 400);
				}
	...
	public function applyPaging($search)
	{
		if (!empty($this->maxLimit)) {
			if (empty($this->limit) || $this->limit > $this->maxLimit) {
				throw new \CHttpException(400, 'You must set a limit value within [1, '.$this->maxLimit.']', 400);
			}
		}
